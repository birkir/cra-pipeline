...
if (WEBPACK_MODE === 'development') {
  const { REMOTE_PORT, HOST = 'localhost' } = process.env;
  Object.keys(cspConfig.directives).forEach(directive => {
    cspConfig.directives[directive].push(`${HOST}:${REMOTE_PORT}`);
    if (HOST !== '0.0.0.0') {
  ...
  cspConfig.directives.scriptSrc.push(
...
const securityMiddleware = [
  ...
  (req, res, next) => {
    ...
    res.locals.nonce = pattern.map(hash).join`-`;
    next();
  },
